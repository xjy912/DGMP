...
def process_result_data(model_name,dataset):
    ...
    if model_name=='{}_DGMP'.format(dataset) or model_name=='{}_GCN'.format(dataset):
    ...
    elif model_name == '{}_NRFD'.format(dataset):
        with open('/home/disk1/xujingyu/桌面/gcn_data/kegg_NRFD.pkl', 'rb') as f:
             y_pred, y_test = pkl.load(f)
        with open('/home/disk1/xujingyu/桌面/gcn_data/predictions_NRFD.pkl', 'rb') as f:
            predictions = pkl.load(f)
        return y_test, y_pred, predictions[0]
    
    elif model_name == '{}_pagerank'.format(dataset):
        path= '/home/disk1/xujingyu/桌面/gcn_data/{}_pagerank.txt'.format(dataset)
        result = pd.read_csv(path,sep='\t')
        result['Gene_ID'] = result['Gene_ID'].astype('category')
        result['Gene_ID'].cat.reorder_categories(node_names.genes, inplace=True)
    ...
    elif model_name=='{}_deepwalk'.format(dataset):
        embedding_file='/home/disk1/xujingyu/桌面/gcn_data/embedding_1'
        deepwalk_embeddings = pd.read_csv(embedding_file, sep='\t',header=None)
        deepwalk_embeddings.drop(axis=0, index=0,inplace=True)
        deepwalk_embeddings = deepwalk_embeddings.iloc[:,0].str.split(' ', expand=True)
        ...
        y_test = labels[mask_test[i].cpu().numpy()]
        #clf = LogisticRegression(class_weight='balanced')
        #clf = RandomForestClassifier()
        clf = SVC(kernel='rbf', class_weight='balanced', probability=True)
        clf.fit(X_train_dw, y_train.reshape(-1))
        y_pred = clf.predict_proba(X_test_dw)
        logits = clf.predict_proba(X_dw)
        y_pred = y_pred[:,1]
        predictions = logits[:,1]
    
        return y_test, y_pred,predictions
    
    elif model_name=='HotNet2':
        ...
        mask = test_mask
        y_pred = pred[mask.astype(np.bool)]
        y_true = labels[mask.astype(np.bool)]
    
        return y_true,y_pred,pred
    
    elif model_name=='MutSigCv':
        input_path = '/home/disk1/xujingyu/桌面/gcn_data/output.txt'
        ...
        #pred = pred.fillna(0)
        pred = np.array(pred,dtype=np.float)
        y_pred = pred[test_mask.astype(np.bool)]
        y_true = labels[test_mask.astype(np.bool)]
